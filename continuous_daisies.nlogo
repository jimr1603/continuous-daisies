...
daisies-own [
  health    ;; Unhealthy plants die. Aging will reduce health, so old plants die. Healthy plants make babies.
  albedo    ;; fraction (0-1) of energy absorbed as heat from sunlight
  preferred-temp ;; temperature the daisy is happiest at
]
...
to setup
  ...
  set solar-luminosity 1
  set global-temperature 0

  ;; spawn initial daisies
  seed-daisies-randomly
  ask daisies [set health init-health
               set albedo random-float 1
               set preferred-temp (random 20) + 30
               ifelse albedo > 0.5 [set color white] [set color black]]

  ask patches [calc-temperature]

  ;;monitors and plots
  set global-temperature (mean [temperature] of patches)
  set mean-albedo (mean [albedo] of daisies)
  set sd-albedo (standard-deviation [albedo] of daisies)
...
to seed-daisies-randomly
  ;;TODO, consider a non-hardcoded 20% of patches get a daisy.
  ask n-of floor(0.20 * count patches) patches with [not any? daisies-here]
    [ sprout-daisies 1 ]
end

to calc-temperature ;; updates patch temperature based on solar lum  & local albedo
  ;;TODO: change colour of patch based on local temperature
  ...
  set pcolor scale-color red temperature -50 110
end
...
to check-survivability ;; make some daisies die of age, heat stress, freezing. All the fun stuff.


  let discomfort  abs(temperature - preferred-temp)
  set health (health - 1 - discomfort) ;; Always decrease health with age.


  if ( health > 20 ) [ seed-daisy ]

  if (health <= 0) [ die ] ;; Goddam I love writing this.
end

to seed-daisy
  if (random 5 = 1 or health <= 30)  ;; throw some randoms. For giggles let dying plants spawn
   [
      ...
      [
        ;; albedo
        let child-albedo albedo + random-normal 0 0.3
        if (child-albedo > 1) [set child-albedo 1]
        if (child-albedo < 0) [set child-albedo 0]

        ; colour
        let child-color black
        if (child-albedo > 0.5 ) [set child-color white]

        ;pref-temp
        let child-temp  preferred-temp  + random-normal 0 1

        ;sprout
        ask seeding-place
        [sprout-daisies 1
             [set health init-health ;; omitting this made babies die so quicky :(
              set albedo child-albedo
              set preferred-temp child-temp
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot mean-albedo"
...
Modified from the Daisyworld Model by Novak & Wilensky:
...
Original model Copyright 2006 Uri Wilensky.
Modifications Copyright 2021 James Riley. 
